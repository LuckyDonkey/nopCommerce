...
using Nop.Data;
using Nop.Plugin.Misc.WatermarkPro.Infrastructure;
...
namespace Nop.Plugin.Misc.WatermarkPro.Services;
public class MiscWatermarkProPictureService : PictureService, IDisposable
{
    ...
    private bool IsPluginInstalled => _pluginService.GetPluginDescriptorBySystemNameAsync<WatermarkProPlugin>("Misc.WatermarkPro") != null;

    public MiscWatermarkProPictureService(
    ...
    private static void PlaceImageWatermark(SKBitmap destImage, SKImage watermarkImage,
        WatermarkProSettings currentSettings)
    {
    ...
    private void PlaceTextWatermark(SKBitmap sourceBitmap, WatermarkProSettings currentSettings)
    {
    ...
    private bool IsWatermarkRequired(int pictureId, WatermarkProSettings settings)
    {
    ...
    private async Task<WatermarkProSettings> GetSettingsAsync()
    {
        var currentStore = await _storeContext.GetCurrentStoreAsync();
        return await _settingService.LoadSettingAsync<WatermarkProSettings>(currentStore.Id);
    }
    ...
    private SKTypeface GetFontTypeface(WatermarkProSettings settings)
    {
    ...
    ~MiscWatermarkProPictureService()
    {
        ReleaseUnmanagedResources();
    }
